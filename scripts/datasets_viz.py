...
SUPPORTED_ROOT_MODALITIES = ("he", "imc", "codex", "cycif", "mibi", "ihc")
SIMPLE_MODALITIES = ("he", "imc", "codex", "cycif", "mibi")
MULTIPLEX_MODALITIES = {"imc", "codex", "cycif", "mibi"}
HIGH_LEVEL_MODALITY_FILTERS = {
    "__multiplex__": "Multiplex",
    "__he__": "H&E",
    "__ihc__": "IHC",
}
...
def normalize_tissues_metadata(metadata: pd.DataFrame) -> pd.DataFrame:
    if "tissue_id" not in metadata.columns and metadata.index.name == "tissue_id":
        metadata = metadata.reset_index()
    return metadata


def count_image_stores(resolution_dir: Path) -> int:
    images_dir = resolution_dir / "images"
    if images_dir.exists():
        return len(list(images_dir.glob("*.ome.zarr"))) + len(
            [path for path in images_dir.glob("*.zarr") if not path.name.endswith(".ome.zarr")]
        )
    return len(list(resolution_dir.glob("*.ome.zarr"))) + len(
        [path for path in resolution_dir.glob("*.zarr") if not path.name.endswith(".ome.zarr")]
    )
...
def discover_standardization_specs(modality_dir: Path) -> list[tuple[str, str, Path]]:
    specs: set[tuple[str, str, Path]] = set()
    if not modality_dir.exists():
        return []

    for resolution_dir in sorted(path for path in modality_dir.iterdir() if path.is_dir() and path.name.endswith("mpp")):
        std_dir = resolution_dir / "standardization"
        if not std_dir.exists():
            continue
        for method_dir in sorted(path for path in std_dir.iterdir() if path.is_dir()):
            if any(method_dir.glob("*.parquet")):
                specs.add((resolution_dir.name, method_dir.name, method_dir))
            for spec_dir in sorted(path for path in method_dir.iterdir() if path.is_dir()):
                if any(spec_dir.glob("*.parquet")):
                    spec = f"{method_dir.name}/{spec_dir.name}"
                    specs.add((resolution_dir.name, spec, spec_dir))
    return sorted(specs, key=lambda item: (item[0], item[1]))
...
def load_dataset_allowlist(path: Path | None) -> set[str] | None:
    if path is None:
        return None
    names: set[str] = set()
...
def modality_filter_options(modalities: list[str]) -> list[tuple[str, str]]:
    return [
        ("All modalities", "__all__"),
        *[(label, value) for value, label in HIGH_LEVEL_MODALITY_FILTERS.items()],
        *[(modality, modality) for modality in modalities],
    ]


def filter_modalities(modality_df: pd.DataFrame, filter_value: str) -> pd.DataFrame:
    if filter_value == "__all__" or modality_df.empty:
        return modality_df
    if filter_value == "__multiplex__":
        return modality_df[modality_df["root"].isin(MULTIPLEX_MODALITIES)]
    if filter_value == "__he__":
        return modality_df[modality_df["modality"] == "he"]
    if filter_value == "__ihc__":
        return modality_df[modality_df["root"] == "ihc"]
    return modality_df[modality_df["modality"] == filter_value]


def filter_std_to_modalities(std_df: pd.DataFrame, modality_df: pd.DataFrame) -> pd.DataFrame:
    if std_df.empty or modality_df.empty:
        return std_df.iloc[0:0].copy() if not std_df.empty else std_df
    allowed = set(zip(modality_df["dataset"].astype(str), modality_df["modality"].astype(str), strict=False))
    mask = [
        (str(row.dataset), str(row.modality)) in allowed
        for row in std_df[["dataset", "modality"]].itertuples(index=False)
    ]
    return std_df.loc[mask].copy()


def recompute_dataset_summary(
    ...
    for _, dataset_row in dataset_df.iterrows():
        ...
        modality_rows = modality_df.loc[modality_df["dataset"] == dataset]
        crop_rows = crop_df.loc[crop_df["dataset"] == dataset] if not crop_df.empty else pd.DataFrame()
        std_rows = std_df.loc[std_df["dataset"] == dataset] if not std_df.empty else pd.DataFrame()

        row = dataset_row.copy()
        row["num_modalities"] = int(len(modality_rows))
        row["loader_modalities"] = ", ".join(modality_rows["modality"].astype(str).tolist())
        row["images_best_resolution_total"] = int(modality_rows["images_best_resolution"].fillna(0).sum()) if not modality_rows.empty else 0
        row["num_standardization_specs"] = int(len(std_rows))

        resolutions: set[str] = set()
        for value in modality_rows["resolutions"].dropna().astype(str):
            resolutions.update(part.strip() for part in value.split(",") if part.strip())
        row["resolutions"] = ", ".join(sorted(resolutions))

        row["num_crop_specs"] = int(len(crop_rows))
        row["total_crops"] = int(crop_rows["num_crops"].fillna(0).sum()) if not crop_rows.empty else 0
        if crop_rows.empty:
            row["crop_counts_by_size"] = "-"
        else:
            counts = crop_rows.groupby("crop_size")["num_crops"].sum().sort_index()
            row["crop_counts_by_size"] = ", ".join(f"{size}:{int(count)}" for size, count in counts.items())

        rows.append(row)

    out = pd.DataFrame(rows).reset_index(drop=True)
    return out[out["num_modalities"] > 0].reset_index(drop=True)


def load_inventory(dataset_allowlist: set[str] | None = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for dataset_dir in sorted(path for path in datasets_dir.iterdir() if path.is_dir()):
        if dataset_allowlist is not None and dataset_dir.name not in dataset_allowlist:
            continue
        ...
        try:
            metadata = normalize_tissues_metadata(pd.read_parquet(metadata_path))
        except Exception:
            continue
        ...
        for modality in loader_modalities:
            ...
            for resolution_dir in resolution_dirs:
                zarr_count = count_image_stores(resolution_dir)
            ...
            if root in MULTIPLEX_MODALITIES and channels_path.exists():
                try:
                    ...
                    if "channel_name" not in channels_df.columns and channels_df.index.name == "channel_name":
            ...
            for std_resolution, spec, std_path in std_specs:
                std_rows.append(
                    {
                        "dataset": dataset_dir.name,
                        "modality": modality,
            ...
            modality_rows.append(
                {
                    ...
                    "standardization_specs": ", ".join(f"{resolution}/{spec}" for resolution, spec, _ in std_specs),
    ...
    if not std_df.empty:
        std_df = std_df.sort_values(["dataset", "modality", "resolution", "standardization_spec"]).reset_index(drop=True)
...
class FilterPanel(Widget):
    ...
    def __init__(self, datasets: list[str], modalities: list[str], **kwargs: object) -> None:
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label("▸ MODALITY", classes="section-label")
        yield Select(modality_filter_options(self._modalities), value="__all__", id="modality-select")
    ...
    @property
    def selected_modality_filter(self) -> str:
        value = self.query_one("#modality-select", Select).value
...
class SummaryView(Widget):
    ...
    def refresh_data(
        ...
        if not crop_df.empty:
            for crop_size, count in crop_df.groupby("crop_size")["num_crops"].sum().items():
                crop_totals_by_size[str(crop_size)] = int(count)
        ...
        self.query_one("#m-images", Static).update(metric(int(modality_df["images_best_resolution"].fillna(0).sum()) if not modality_df.empty else 0, "images"))
        self.query_one("#m-crops", Static).update(metric(int(len(crop_df)), "tile files"))
        self.query_one("#m-std", Static).update(metric(int(len(std_df)), "std specs"))
        for crop_size in self._crop_sizes:
    ...
    def _render_modality(
        ...
        *,
        ...
        if std_rows.empty:
            ...
            std_table.add_columns("Resolution", "Spec")
            for _, std_row in std_rows.iterrows():
                std_table.add_row(str(std_row["resolution"]), str(std_row["standardization_spec"]))
...
class DatasetInventoryApp(App):
    ...
    def __init__(self, dataset_allowlist: set[str] | None = None) -> None:
        super().__init__()
        self._dataset_allowlist = dataset_allowlist
    ...
    def compose(self) -> ComposeResult:
        ...
        self._dataset_df, self._modality_df, self._crop_df, self._std_df = load_inventory(self._dataset_allowlist)
        ...
        modalities = sorted(self._modality_df["modality"].astype(str).unique().tolist()) if not self._modality_df.empty else []

        with Horizontal():
            yield FilterPanel(datasets=datasets, modalities=modalities, id="filter-panel")
    ...
    def _refresh_all(self) -> None:
        ...
        modality_filter = filter_panel.selected_modality_filter
        ...
        modality_df = filter_modalities(modality_df, modality_filter)
        keep_datasets = set(modality_df["dataset"].astype(str).tolist()) if not modality_df.empty else set()
        dataset_df = dataset_df[dataset_df["dataset"].astype(str).isin(keep_datasets)]
        if not crop_df.empty:
            crop_df = crop_df[crop_df["dataset"].astype(str).isin(keep_datasets)]
        if not std_df.empty:
            std_df = filter_std_to_modalities(std_df, modality_df)

        dataset_df = recompute_dataset_summary(dataset_df, modality_df, crop_df, std_df)
        ...
        self.query_one("#summary-view", SummaryView).refresh_data(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize SPORA dataset inventory in a Textual TUI.")
    parser.add_argument(
        "--dataset-list",
        type=Path,
        default=None,
        help="Optional text file containing one dataset name per line. Only the intersection with on-disk datasets is shown.",
    )
    args = parser.parse_args()
    DatasetInventoryApp(dataset_allowlist=load_dataset_allowlist(args.dataset_list)).run()
