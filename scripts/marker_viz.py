...
def load_inventory() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for dataset_dir in sorted(p for p in datasets_dir.iterdir() if p.is_dir()):
        ...
        for modality in MULTIPLEX_MODALITIES:
            ...
            if "channel_name" not in df.columns and df.index.name == "channel_name":
                df = df.reset_index()
            ...
            if "is_nuclear_marker" in df.columns:
                explicit_nuc = df["is_nuclear_marker"].fillna(False).astype(bool)
                nuc = nuc | explicit_nuc
                reason.loc[explicit_nuc & (reason == "")] = "is_nuclear_marker column"
            has_uni = df["uniprot_id"].map(is_real_uniprot)
            channel_index = df["index"].astype(int).tolist() if "index" in df.columns else list(range(len(df)))
